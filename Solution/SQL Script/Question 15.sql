SELECT 
    ...
    CASE 
        ...
        'heavy thunderstorm', 'thunderstorm with rain', 'thunderstorm with light rain', 'thunderstorm with light drizzle') 
        THEN 'Severe'
        WHEN f.weather_description IN ('storm', 'heavy intensity rain', 'heavy snow', 
        'heavy shower snow', 'heavy intensity shower rain', 'heavy intensity drizzle', 'heavy intensity drizzle rain', 'sleet', 'freezing rain', 'squalls') 
        THEN 'Moderate'
        ...
        'proximity shower rain', 'proximity moderate rain', 'proximity thunderstorm', 'proximity thunderstorm with drizzle', 'proximity thunderstorm with rain') 
        THEN 'Low'
        ELSE 'Normal'
...
WHERE 
    ...
    AND f.wind_direction IS NOT NULL 
    AND f.weather_description IS NOT NULL 
    AND f.wind_speed IS NOT NULL
    AND 
    EXISTS (
        SELECT 1 
